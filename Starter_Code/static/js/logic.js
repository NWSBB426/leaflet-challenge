...
//URL of the earthquake data JSON
...
// Event listener for depth filter
document.getElementById('depthFilter').addEventListener('change', (event) => {
    filterMapByDepth(event.target.value);
});

function filterMapByDepth(depth) {
    ...
    // Fetch earthquake data and apply filter
    d3.json(earthquakeDataUrl)
        .then(data => {
            data.features.forEach(feature => {
                var coords = feature.geometry.coordinates;
                var depthValue = coords[2]; // Depth is the third coordinate

                // Check if the depth of the earthquake matches the selected filter
                if ((depth === 'shallow' && depthValue < 70) ||
                    (depth === 'intermediate' && depthValue >= 70 && depthValue <= 300) ||
                    (depth === 'deep' && depthValue > 300)) {
                    ...
                    var depthColors = d3.scaleLinear()
                        .domain([0, 70, 300]) // Depth ranges
                        .range(['#8B0000', '#FFFF00', '#00FFFF']); // Corresponding colors
                    var markerColor = depthColors(depthValue);

                    // Create and add circle markers to the map
                    L.circleMarker([coords[1], coords[0]], {
                        radius: markerSize,
                        fillColor: markerColor,
                        color: '#000',
                    ...
                    .bindPopup('Magnitude: ' + magnitude + '<br>Depth: ' + depthValue + ' km<br>Location: ' + title)
                    .addTo(map);
                }
            });
        })
